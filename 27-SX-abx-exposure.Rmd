...
write.csv(sample_data(mpa_phy_colitis) %>% as_tibble() %>%  select(anonymized_id, .sample, case ) %>% left_join(colitis_metadata %>% select( mrn, anonymized_id) %>% distinct() ), file = "colitis_noColitis_sample_metadata.csv")
